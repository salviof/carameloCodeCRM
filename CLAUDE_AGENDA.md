...
`FluxoAgendaDoConsultor` (tag `@AgendaConsultorDefinirAgenda`): passos `A`–`J`
passando. O passo `K_Quando__uma_reserva_e_realizada_para_segunda_as_8_horas`
está inacabado — lê a lista via `getCPinst(CPEscopoPesquisaMelhorHorario
.listahorariosdisponiveis)` (CP do CRM sobre um `AgendaDisponibilidade`, retorna
`String` → `ClassCastException`), chama `limparAtendenteDefinido()` logo antes de
ler, e tem `ItfResposta resp = null` com a chamada real comentada. `L` e `M`
ficam como *skipped* em consequência.
